...
1. **Part-of-Speech (POS) Tagging**: Assigning grammatical roles (Noun, Verb, etc.) to words.
2. **Named Entity Recognition (NER)**: Extracting entities (Persons, Organizations, Locations) from text.
...
* **Source:** [Universal Dependencies v2.15 - Russian SynTagRus](https://github.com/UniversalDependencies/UD_Russian-SynTagRus)
...
* **Source:** [Hugging Face WikiANN (Russian subset)](https://huggingface.co/datasets/wikiann)
...
| :--- | :--- | :--- |
| **Unigram** | 83.87% | Baseline. Failed on ambiguous words. |
| **HMM** | 87.38% | Improved context but failed on OOV words. |
| **BiLSTM** | ~80.14% | Suffered from lack of pre-training (vocabulary sparsity). |
| **spaCy** | ~94.00% | Strong industrial performance. |
| **CRF** | **96.43%** | **Winner (Tie).** Manual suffix features solved the morphology problem. |
| **RoBERTa** | **96.42%** | **Winner (Tie).** Deep semantic understanding matched explicit rules. |

#### 🔍 Qualitative Findings (POS)
1. **The "Slang" Test:** On the neologism *"Zaguglil"* (Googled), statistical models failed. The **CRF** succeeded because it recognized the verbal suffix **-il**.
2. **The "Homonym" Test:** On the sentence *"Mother started to **bake** (pech) pies... stood a **stove** (pech)"*, the **RoBERTa** model was the *only* one to correctly distinguish the Verb from the Noun using self-attention.
...
| :--- | :--- | :--- |
| **HMM** | 73.91% | **Failed.** Relied on memory; catastrophic failure on unseen names ("Zombie Name" effect). |
| **CRF** | 85.13% | **Good.** High score on test set, but overfit to Wikipedia title structures. |
| **RuBERT** | **91.17%** | **SOTA.** The only model to understand complex sentence structures. |

#### 🔍 Qualitative Findings (NER)
1. **The "Elon Musk" Test:** Input: *"Elon Musk bought Twitter"*.
...
2. **The "Ambiguity" Test:** Input: *"Ivan reads a novel"* (Roman chitayet roman).
